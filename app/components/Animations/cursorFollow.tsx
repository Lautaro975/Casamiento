...
export default function ImageTrail({ items = [], variant = 1 }: ImageTrailProps): JSX.Element {
  ...
  return (
    <div className="w-full h-full relative rounded-lg bg-transparent overflow-visible pointer-events-auto" ref={containerRef}>
      {items.map((url, i) => (
        <div
          className="content__img w-[190px] aspect-[1.1] rounded-[20px] absolute top-0 left-0 opacity-0 overflow-hidden [will-change:transform,filter]"
          key={i}
        >
          <div
